...
int main()
{
	wstring outputFile = OUTPUTPATH"CreateTreeMapChart.pptx";

	//Create a PPT document
	intrusive_ptr<Presentation> ppt = new Presentation();

	//Create a TreeMap chart to the first slide
	intrusive_ptr<IChart> chart = ppt->GetSlides()->GetItem(0)->GetShapes()->AppendChart(ChartType::TreeMap, new RectangleF(50, 50, 500, 400), false);
	...
	std::vector<std::vector<std::wstring>> categories =
	{
		...
		{L"Branch 1", L"Stem 2", L"Leaf 6"},
		{L"Branch 1", L"Stem 2", L"Leaf 7"},
		{L"Branch 2", L"Stem 3", L"Leaf 8"},
		{L"Branch 2", L"Stem 3", L"Leaf 9"},
	...
	chart->GetSeries()->GetItem(0)->SetTreeMapLabelOption(TreeMapLabelOption::Banner);
	chart->GetChartTitle()->GetTextProperties()->SetText(L"TreeMap");
