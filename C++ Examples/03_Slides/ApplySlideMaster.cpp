...
int main()
{
	...
	std::wstring outputFile = OutputPath"ApplySlideMaster.pptx";
	...
	IMasterSlide* masterSlide = ppt->GetMasters()->GetItem(0);

	//Customize the background of the slide master
	std::wstring backgroundPic = DataPath"bg.png";
	...
	IEmbedImage* image = masterSlide->GetShapes()->AppendEmbedImage(ShapeType::Rectangle, backgroundPic.c_str(), rect);
